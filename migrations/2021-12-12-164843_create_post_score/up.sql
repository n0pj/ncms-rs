...
CREATE TABLE IF NOT EXISTS `database`.`post_score` (
  ...
  `post_score_type_id` VARCHAR(45) NOT NULL,
  `post_status_uuid` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`uuid`),
  UNIQUE INDEX `id_UNIQUE` (`uuid` ASC),
  INDEX `fk_post_score_post_score_type1_idx` (`post_score_type_id` ASC),
  INDEX `fk_post_score_post_status1_idx` (`post_status_uuid` ASC),
  CONSTRAINT `fk_post_score_post_score_type1`
    FOREIGN KEY (`post_score_type_id`)
    REFERENCES `database`.`post_score_type` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
